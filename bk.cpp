...
int main()
{
	...
	ofstream fout3("bk_EE_trans_accuracy-res.txt",ios::app);
	for(int w=250;w<500;w+=50)
	{ 
		ifstream fin("EE2.txt");
		//ifstream fcheck("ws2check.txt");
		//ifstream fsrc("ws2srcnode.txt");
		ifstream ftrans("transquery-EE.txt");
		int uns_w = w;
	...
	//	ftrans>>v;
		...
		for (int i = 0; i<n; i++)
		{
			uns_res = uns->transquery(str1[i], str2[i]);
			if (uns_res) n1++;//代表误报
		}
		finish = clock();
		d3 = double(finish - start) / CLOCKS_PER_SEC;
